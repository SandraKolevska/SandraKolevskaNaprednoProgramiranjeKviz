...
char tocniPrvi[4]={'b', 'a', 'c', 'a'};
char tocniVtori[4]={'b', 'c', 'a', 'b'};
char tocniTreti[4][2]={{'a', 'd'}, {'c', 'e'}, {'b', 'e'}, {'c', 'e'}};
...
void logStudentData(student lice, int studentNumber) {
    time_t vreme;
    ofstream logFile("Students.log", ios::app);
    logFile << studentNumber << ". Vreme: " << asctime(localtime(&vreme));
    logFile << "   Ime: " << lice.ime << endl;
    logFile << "   Prezime: " << lice.prezime << endl;
    logFile << "   Indeks: " << "INKI" << lice.indeks << endl;
    logFile.close();
}
...
int main() {
    ...
    do{
        ...
        logStudentData(lice, studentNumber);
        studentNumber++;
        brojnaprasanje=1;
        poeni=0;
